...
### 도움말 오버레이(F1)
- F1 키로 중앙 도움말 패널을 토글합니다. 패널은 최소 명령 요약을 창 형태로 보여주며 `clear_under`를 사용해 뒤에 있는 로그와 자연스럽게 겹칩니다.
- 창 주변에는 한 줄 정도의 공백만 두어 전체 화면을 가리지 않고, ESC/F1을 다시 누르면 즉시 복귀합니다.
- 안내 텍스트는 `/login`, `/join`, `/whisper` 등 주요 슬래시 명령을 포함하도록 갱신됩니다 (2025-10).
...
```json
{
  "commands": [
    ...
    {"name": "/whisper",    "args": ["<user>", "<message...>"],"desc": "로그인 사용자 간 귓속말(서버 로그 기록)"},
