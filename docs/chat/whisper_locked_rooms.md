...
> Status (2026-03-07): Design note / feature plan, not a statement of fully shipped behavior.
...
- 사용자 목록: 잠금 방은 비회원 조회(`/who`)를 허용하지 않으며, 클라이언트 UI(`client_gui`)에서는 `🔒 room-name` 형식으로 표시할 수 있다.
...
- [ ] 클라이언트(`client_gui`/CLI) 명령 파서 및 로그 컬러링 확장
