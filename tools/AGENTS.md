...
- `tools/admin_app/main.cpp` + `tools/admin_app/admin_ui.html`: admin control-plane (read + write-lite) (`/metrics`, `/healthz`, `/readyz`, `/admin`, `/api/v1/auth/context`, `/api/v1/overview`, `/api/v1/instances`, `/api/v1/users`, `/api/v1/sessions/{client_id}`, `/api/v1/users/disconnect`, `/api/v1/users/{mute|unmute|ban|unban|kick}`, `/api/v1/announcements`, `/api/v1/settings`, `/api/v1/worker/write-behind`, `/api/v1/metrics/links`).
...
- `tools/check_lua_build_toggle.py`: verifies `BUILD_LUA_SCRIPTING` cache value and Lua runtime source selection (`lua_runtime.cpp` vs `lua_runtime_disabled.cpp`).
