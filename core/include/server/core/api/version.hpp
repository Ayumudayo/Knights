...
inline constexpr std::uint32_t k_version_minor = 5;
/** @brief 공개 API 시맨틱 버전의 패치 값입니다. */
inline constexpr std::uint32_t k_version_patch = 1;

/**
 * @brief 공개 API 표면의 시맨틱 버전 문자열을 반환합니다.
 * @return `major.minor.patch` 형식의 SemVer 문자열 리터럴
 */
inline constexpr std::string_view version_string() noexcept { return "1.5.1"; }
