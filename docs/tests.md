...
- world residency fallback proof는 persisted world key를 제거한 뒤 `default world + lobby` safe fallback이 적용되는지 확인한다.
- world owner fallback proof는 persisted world owner key를 다른 backend owner로 바꾼 뒤에도 restore가 `default world + lobby` safe fallback으로 접히고 owner key가 현재 backend로 복구되는지 확인한다.
