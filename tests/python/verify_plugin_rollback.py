...
def apply_swap(staging_path: str) -> None:
    swap_script = (
        "set -e; "
        f'test -f "{staging_path}" || (echo "missing staging plugin: {staging_path}" >&2; exit 1); '
        f'touch "{LOCK_PATH}"; '
        f'cp "{staging_path}" "{ACTIVE_PLUGIN_PATH}"; '
        f'rm -f "{LOCK_PATH}"'
    )
...
def main() -> int:
    try:
        # Normalize to v2 first so rollback always verifies a real transition.
        apply_swap(STAGING_V2_PATH)
        wait_for_version("v2")

        # Roll back to v1 and verify convergence.
        apply_swap(STAGING_V1_PATH)
        wait_for_version("v1")

        print("PASS: plugin rollback to v1 reflected in metrics")
        return 0
