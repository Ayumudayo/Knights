...
| `SERVER_REGISTRY_PREFIX` | `gateway/instances/` | Redis Instance Registry prefix (Gateway가 읽는 prefix와 일치해야 함. 현재 `gateway_app`은 `server:registry:` prefix를 사용하므로, `SERVER_REGISTRY_PREFIX=server:registry:`로 맞추는 것을 권장) |
| `SERVER_REGISTRY_TTL` | `30` | Instance Registry TTL(초) |
| `SERVER_HEARTBEAT_INTERVAL` | `5` | Instance Registry heartbeat 주기(초) |
| `METRICS_PORT` | `0` | HTTP metrics 포트(관측/대시보드 용도). 0이면 비활성 |
